...
def output(filename):
    ...
    #video = cv2.VideoWriter('traffic_counter3.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps, (height2, width2),
                           # 1)
    filename = 'traffic_counter4.mp4'
    video = cv2.VideoWriter('traffic_counter4.mp4',cv2.VideoWriter_fourcc(*'MP4V'),fps,(height2,width2),1)
    ...
    def postProcess(outputs, image):
        ...
        for i in indices.flatten():
            ...
            cv2.putText(image, f'{name.upper()} {int(confidence_scores[i] * 100)}%',
...
def displayOutput(finalop):
    ...
    clip = mp.VideoFileClip("traffic_counter4.mp4")
    clip_resized = clip.resize(height=360)
    clip_resized.write_videofile("Resized.mp4")
    st.video('Resized.mp4', 'rb')
    print("Work2")
    exit(0)
